...
} from "@/components/global/elements/boxes/breadcrumb";
...
const SingleArticlePageHeader = ({
  ...
  return (
    <header
      className={` lg:bg-position-[center_top_-8rem]
            ${cls} overflow-hidden sm:h-120 h-100 bg-cover relative w-full
        `}
    >
      <Image
        src={imgBgSrc}
      ...
      <Navbar isWhite={true} />
      <div
        className={`text-white sm:px-26 sm:py-16 px-12 py-8 flex flex-col md:items-start items-center justify-center xl:my-24 my-20 xs:rounded-[5rem] rounded-[3rem]`}
      >
        {/* <div
                 className={`
        ...
        <div>{/* {badgeTextTranslator(category)} */}</div>
        <h2 className="sm:text-[3.5rem] text-[1.9rem] tracking-tight leading-tight! font-black md:text-right text-center z-10">
          {title}
        </h2>
        <div className="flex mt-3">
          ...
          <div className="flex items-center mr-12">
            ...
            <Paragraph
              cls="font-medium text-sm mt-0.5 z-0 text-white"
              text={date}
            />
          </div>
        </div>

        ...
        <>
            ...
            <section className="mb-5">
                ...
                <div className="mt-24 grid grid-cols-12 sm:gap-x-6 gap-y-16 mx-3">
                    {
                        isLoading
                        ? <CircleLoading text="کاربرگ ها در حال بارگذاری هستند ..." cls="col-span-12" />
                        : error
                            ? <ErrorShower text="مشکلی در دریافت اطلاعات به وجود آمده است !"/>
                            : data?.data?.map((item, index) => {
                                return (
                                    <SheetsCard grade={params?.grade} cls="xl:col-span-3 lg:col-span-4 sm:col-span-6 col-span-12 " item={item} key={index}/>
                                )
                            })
                    }
                </div>
                {
                    Number(data?.totalPages) > 1 && <ListPagination params={params} setPage={setPage} totalPages={data?.totalPages}/>
                }
            </section>
        </>

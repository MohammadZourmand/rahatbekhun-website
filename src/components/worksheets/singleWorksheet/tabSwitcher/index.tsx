...
import Similarsheets from './info';
...
const TabSwitcher = ({data} : {data : worksheetsDataProps}) => {
  ...
  const categories : TabSwitcherCategoriesProps[] = [
    {id : 0, text : "کاربرگ های مشابه", icon : <DocumentBrokenIcon cls={`${selectedTab === 0 ? "fill-white" : "fill-gray-800" } !w-6 !h-6 transition duration-300`} />},
    {id : 1, text : "دیگران دیده اند", icon : <FolderBroken cls={`${selectedTab === 1 ? "fill-white" : "fill-gray-800" } !w-6 !h-6 transition duration-300`} />},

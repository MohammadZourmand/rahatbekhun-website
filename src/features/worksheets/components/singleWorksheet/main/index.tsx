...
const SingleWorksheetMain = ({ data }: { data: worksheetsDataProps }) => {
  ...
  console.log(
    "http://localhost:5000" + data.pdf.split("https://rahatbekhun.ir")[1],
  );
  ...
  return (
    <main className="realtive xl:mx-auto mx-2 grid grid-cols-5 max-w-7xl gap-4 lg:mt-0 mt-16">
      ...
      <Image
        ...
        width={55}
        height={191}
        className="absolute 2xl:top-144 lg:top-160 lg:block hidden right-0 rotate-180"
      />
      <div className="lg:col-span-1 sm:col-span-2 col-span-5 z-10 mx-auto">
        <img
          src={
            "http://localhost:5000" +
            data?.image.split("http://rahatbekhun.ir")[1]
          }
          alt={data?.name}
      ...
      <div className="lg:col-span-3 col-span-5 lg:-order-none order-first">
        <div className="flex items-center text-gray-400 gap-x-1">
          <span className="hidden xs:inline">
            {" "}
            کاربرگ {data._subject} پایه {data?._grade} /{" "}
          </span>
          <span> {data?.season} </span>
          <span
            className={`px-2 py-0.5 rounded text-sm block ${badgeBgColorChooser(data?.level)} text-white`}
          >
            {data?.level}
          </span>
        </div>
        ...
        <div className="grid grid-cols-4 gap-2 mt-7">
          <div className="bg-gray-100/90 hover:bg-gray-600 transition group rounded-lg text-sm sm:col-span-1 col-span-2 gap-y-2 flex flex-col p-2">
            <span className="text-gray-400 text-sm transition group-hover:text-gray-200">
              نویسنده :
            </span>
          ...
          <div className="flex">
            {data?.hashtags?.map((value, index) => (
              <h2
                className="bg-gray-100 hover:bg-gray-600 hover:text-white transition rounded-lg text-sm py-1 px-3"
                key={index}
              >
                {value}
              </h2>
      ...
      <div className="lg:col-span-1 sm:col-span-3 col-span-5 p-3">
        <div className="flex items-center">
          ...
          <div className="flex flex-col mr-3">
            <span className="text-gray-700 text-md font-extrabold">
              {" "}
              {data?.author}{" "}
            </span>
            <span className="text-gray-400 mt-1 text-xs font-semibold">
              {" "}
              {data?._author}{" "}
            </span>
          </div>
        ...
        {Number(data?.price) === 0 ? (
          <IconBtn
            ...
            href={
              "http://localhost:5000" +
              data?.pdf.split("https://rahatbekhun.ir")[1]
            }
            target={"_blank"}
          />
        ) : (
          <IconBtn
            iconName="cart"
            text={`خرید کاربرگ (${convertEnToPe(data?.price)} تومان)`}
